...
describe('AgentChat', () => {
  ...
  it('shows clarification for out-of-domain queries', async () => {
    ...
    expect(await screen.findByText(/only answer questions about real estate/i)).toBeInTheDocument();
  });
  ...
  it('never provides off-topic or unsupported answers', async () => {
    ...
    for (const query of offTopicQueries) {
      ...
      const clarifications = await screen.findAllByText(/only answer questions about real estate/i);
