...
const VoiceFeedback: React.FC<VoiceFeedbackProps> = ({ listening, error, transcript }) => {
  if (error) {
    return (
      <div style={{
        position: 'absolute',
  ...
  if (listening) {
    return (
      <div style={{
        ...
        Listening...
        {transcript && (
          <span style={{ fontWeight: 400, fontSize: 14, marginLeft: 12, color: '#1976d2' }}>{transcript}</span>
        )}
      </div>
    );
  }
